...
function load_preferences(uuid::UUID)
    ...
    return Base.get_preferences(uuid)
end
...
function save_preferences!(uuid::UUID, prefs::Dict)
    ...
    if !haskey(project, "compile-preferences")
        project["compile-preferences"] = Dict{String,Any}()
    end
    if !isa(project["compile-preferences"], Dict)
        error("$(proj_path) has conflicting `compile-preferences` entry type: Not a Dict!")
    end
    project["compile-preferences"][string(uuid)] = prefs
...
function clear_preferences!(uuid::UUID)
    ...
    if proj_path !== nothing && isfile(proj_path)
        project = TOML.parsefile(proj_path)
        if haskey(project, "compile-preferences") && isa(project["compile-preferences"], Dict)
            delete!(project["compile-preferences"], string(uuid))
            open(proj_path, "w") do io
                TOML.print(io, project, sorted=true)
            end
        end
